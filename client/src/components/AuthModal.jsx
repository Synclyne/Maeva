...
export default function AuthModal({ onClose, defaultTab = 'login' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Authentication">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} aria-hidden="true" />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 overflow-y-auto max-h-[90vh]">
        {/* Close */}
        <button onClick={onClose} aria-label="Close" className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-500">✕</button>
        ...
        {tab === 'forgot' && (
          <div className="mb-5">
            <h2 className="font-display text-xl font-semibold text-gray-900">Forgot Password?</h2>
        ...
        {!success && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {tab === 'register' && (
              <>
                <div>
                  ...
                  <input className="input" placeholder="John Mwangi" value={form.name} onChange={e => set('name', e.target.value)} required />
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {['client','realtor'].map(r => (
                      <button type="button" key={r} onClick={() => set('role', r)}
                        ...
                        <div className="text-xl mb-1">{r === 'client' ? '🏡' : '🏢'}</div>
                ...
                {form.role === 'realtor' && (
                  <div>
                    ...
                    <input className="input" placeholder="e.g. Amani Properties Ltd" value={form.company} onChange={e => set('company', e.target.value)} />
                  </div>
                )}
                <div>
                  <label className="label">Phone Number</label>
                  <input className="input" placeholder="+254 7XX XXX XXX" value={form.phone} onChange={e => set('phone', e.target.value)} />
                </div>
              </>
            )}
            ...
            <button type="submit" disabled={loading} className="w-full btn-primary py-3 rounded-xl font-semibold text-base disabled:opacity-60">
